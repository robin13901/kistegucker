...
export async function POST(request: Request) {
  ...
  if (new Date(performance.start_datetime).getTime() < Date.now()) {
    return NextResponse.json({ error: 'Für vergangene Aufführungen sind keine Online-Reservierungen möglich.' }, { status: 400 });
  }

  const { data: incremented } = await supabase.rpc('increment_reserved_tickets', {
    performance_id_input: parsed.data.eventId,
    ticket_amount: parsed.data.tickets
  });

  if (!incremented) {
    return NextResponse.json({ error: 'Für diese Aufführung sind online nicht mehr genug Tickets verfügbar.' }, { status: 400 });
  }
  ...
  const { error } = await supabase.from('reservations').insert({
    ...
    performance_id: parsed.data.eventId
  });
